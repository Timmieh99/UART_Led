...
RCC->APB1ENR |= 0x20000; //Aktiverar UART2 genom att ändra bit 17 i APB1ENR till 1.

// 2. Enablea klocktillgång för portA

RCC->AHB1ENR |= 0x01; //Aktiverar GPIOA genom att ändra bit 0 i AHB1ENR till 1.
...
USART2->CR1 = 0x000C; //Sätter vi tx och rx till att arbeta i 8 bitars data. (8 bitars data, 1 stop bit, ingen paritet) (aktiverar sändning och mottagning av data i 8bits läge)
USART2->CR2 = 0x000; //Nollställer CR2 alltså ställer in allt till standardkonfigurationen
USART2->CR3 = 0x000; //Nollställer CR3 alltså ställer in allt till standardkonfigurationen
USART2->CR1 |= 0x2000; //Ändrar bit 13 (UART-Aktiveringen) till 1
